...
namespace {
    ...
    inline size_t TypeCount(const std::string& typeStr) {
        // "SCALAR", "VEC2", "VEC3", "VEC4", "MAT2", "MAT3", "MAT4"
        if (typeStr == "SCALAR") return 1;
        if (typeStr == "VEC2") return 2;
        if (typeStr == "VEC3") return 3;
        if (typeStr == "VEC4") return 4;
        if (typeStr == "MAT2") return 4;
        if (typeStr == "MAT3") return 9;
        if (typeStr == "MAT4") return 16;
        return 0;
    }
    ...
    std::vector<float> ReadFloatAccessor(const tinygltf::Model& model, const tinygltf::Accessor& acc) {
        size_t byteStride = 0;
        const unsigned char* data = GetAccessorDataPointer(model, acc, byteStride);
        size_t compCount = TypeCount(acc.type);
        if (!data) return {};
