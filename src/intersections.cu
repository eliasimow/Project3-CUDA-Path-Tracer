...
__host__ __device__ bool intersectAABB(const Ray ray, const float t, const glm::vec3 bmin, const glm::vec3 bmax)
{
...
__host__ __device__ float intersectBVH(
	...
	while (stackIndex > 0) {
		...
		if (runBVH && !boxIntersect) continue;
		...
		if (node.primCount > 0)
		{
			...
			stack[stackIndex++] = node.leftOrFirstTri;
			stack[stackIndex++] = node.leftOrFirstTri + 1;
		}
	}
	return t;
}
